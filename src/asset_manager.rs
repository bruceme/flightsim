...
pub fn load_textures<P: AsRef<Path>>(gl: &GlContext, textures: &[P]) -> Vec<Texture> {
...
pub fn load_object(
    ...
    let textures = load_textures(gl, texture_files);

    Model::new(
        gl,
        Mesh::new(vert, tex, norm, model.indices),
        load_shaders(gl, vert_shader, frag_shader),
        textures,
    )
}
